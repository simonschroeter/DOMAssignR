...
recalEval <- function(import,
  ...
  print(ifelse(any(sapply(recal_result, function(x){x[[3]]}) != "Successfully recalibrated."),
  ...
  #}
  #
  if(any(sapply(recal_result, function(x){x[[3]]}) != "Successfully recalibrated.")){
  for (i in 1:length(recal_result)){
    if(i==1){print("The following samples could not be recalibrated:")}
    if(recal_result[[i]][[3]] != "Successfully recalibrated."){
      print(paste0("Sample",i, ": ", recal_result[[i]][4]))
    }}
  if(!any(sapply(recal_result, function(x){x[[3]]}) != "Successfully recalibrated.")) {print("none")}
  }
